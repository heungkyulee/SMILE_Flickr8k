...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    elif dataset == 'caption_flickr8k':
        from .flickr8k_dataset import Flickr8kDataset
        transform_train = get_transform(config['image_size'], is_train=True, min_scale=min_scale)
        transform_val = get_transform(config['image_size'], is_train=False)
        
        train_dataset = Flickr8kDataset(
            ann_file=os.path.join(config['ann_root'], config['train_file']),
            transform=transform_train,
            image_root=config['image_root'],
            is_train=True  # 학습 모드
        )
        val_dataset = Flickr8kDataset(
            ann_file=os.path.join(config['ann_root'], config['val_file']),
            transform=transform_val,
            image_root=config['image_root'],
            is_train=False  # 평가 모드
        )
        test_dataset = Flickr8kDataset(
            ann_file=os.path.join(config['ann_root'], config['test_file']),
            transform=transform_val,
            image_root=config['image_root'],
            is_train=False  # 평가 모드
        )
